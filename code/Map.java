...
public class Map {
    ...
    public void createFX() {
        ...
        btRun.setOnAction(e -> {
            ...
            TableEntry result = graph.calculateResult(source, destination, filter);

            if (result == null || !result.getRoute().getFirst().getDestination().getCapital().getCapitalName().equals(source.getCapital().getCapitalName())) {
            ...
            Node node = result.getRoute().getFront();
    ...
    public ScrollPane buildMap() {
        // Load the map image
        Image mapImage = new Image("../images/World_Map.jpg");
        ...
        for (Vertex vertex : cbSource.getItems()) {
            ...
            ivPin.setOnMouseClicked(e -> {
                if (cbSource.getValue() == null || !flag) {
